...
在上面的 Releases 页面选择最新的 **v4 Legacy Cache Preview**。下载附件中的 `VRCX-Insights_2026.09.20-cache-v4_Setup.exe` 安装版，或 `VRCX-Insights_2026.09.20-cache-v4.zip` 解压版。不要下载页面底部的 Source code 来运行。
...
v4 增加了**只读旧库兼容层**。开始首次分析时会检查当前源数据库的表名和字段，而不是要求你先用外部工具把老库改成新库。识别到历史官方布局后，界面会显示“旧版格式 · 已兼容转换”，并把旧字段投影成当前分析格式后写入独立的 `analysis-v1.db`。

兼容层目前覆盖这类历史差异：`created_at / createdAt`、`display_name / displayName`、`user_id / userId`、`world_name / worldName / world_id`、旧事件类型名称、没有显式 `id` 时的 SQLite `rowid`，以及旧记录中 `显示名 (usr_xxx)` 这种把用户 ID 混在名字里的格式。已知的无时区 VRCX 时间按 UTC 解释；历史导出若使用 Unix 秒/毫秒也会规范为 UTC 毫秒。

**这里的“转换”只发生在分析缓存里。原始数据库不会 ALTER、UPDATE、VACUUM 或覆盖。** 自动化测试会对源文件做 SHA256 前后比较。遇到无法安全解释的极老结构（例如连事件时间或加入/离开类型都没有）会明确停止并要求保留/重建数据，不会猜字段后破坏源库。

如果你的两年数据一直跟着官方 VRCX 升级、现在仍在同一个 SQLite 文件里，通常直接首次分析即可；新旧行会一起进入缓存。当前仍然是**一个源数据库文件对应一个缓存**，不会自动把多个独立备份文件合并成一条历史。多个旧备份合并属于后续独立功能。
...
**提示 native v3/v4 cache module missing。** 需要完整 EXE 与 HTML 来自同一版本；只替换前端文件不能补上新的 C# 后端。当前可下载测试目标为 Windows x64 CEF，未完成 macOS/Linux Electron 原生桥接与安装验证。
