        ...
        <template v-if="opened && status?.phase === 'ready' && !busy">
            ...
            <div v-if="report && !error" class="ic-grid">
                ...
                <aside class="ic-right">
                    <article class="ic-card"><h2>{{ viewMode === 'group' && !groupMember ? selectedGroup?.name : report.members[0]?.name }}</h2>
                        <div v-for="member in report.members" :key="member.id" class="ic-member"><button v-if="viewMode === 'group' && !groupMember" type="button" @click="groupMember = member.id">{{ member.name }} →</button><strong v-else>{{ member.name }}</strong><p>{{ duration(member.observedMs) }} · {{ member.completeSessions }} {{ s('个完整片段', 'complete sessions') }}</p><small>{{ member.incompleteSessions }} {{ s('个未闭合片段（不补算时长）', 'unclosed records (not extrapolated)') }}</small></div>
                    </article>
                    <article class="ic-card ic-pairs"><h2>{{ reviewIds.length > 1 ? s('组内两两共同游玩', 'Pairwise co-presence within group') : s('共同在场的人', 'Observed companions') }}</h2><p>{{ s('只算完整记录在同一观察场次、同一实例中的时间交集；不代表关系或实际互动。', 'Only complete, same-visit, same-instance overlaps count; this is not proof of interaction or a relationship.') }}</p>
                        <div v-for="pair in report.pairs.rows" :key="`${pair.leftId}|${pair.rightId}`" class="ic-pair"><div><button v-if="reviewIds.length > 1" type="button" @click="choose(pair.leftId, pair.leftName)">{{ pair.leftName }}</button><span v-if="reviewIds.length > 1"> ↔ </span><button type="button" @click="choose(pair.rightId, pair.rightName)">{{ pair.rightName }}</button></div><strong>{{ duration(pair.observedMs) }}</strong><small>{{ pair.segments }} {{ s('段', 'segments') }}</small></div>
...
const viewKey = computed(() => JSON.stringify([account.value, reviewIds.value, appliedRange.value]));
...
function openCache() { opened.value = true; error.value = ''; notice.value = ''; appliedRange.value = cacheRange(rangeKind.value, customStart.value, customEnd.value); }
...
async function loadBlock(kind) {
    ...
    const options = { ...appliedRange.value, userIds: [...reviewIds.value], page: configs[kind].page, size: configs[kind].size };
